...
trait Repository {
  ...
  fn query(&self, id: Self::IdType) -> Result<Self::Entity> {
    let entity = self.query_optional(id).map_err(Err(anyhow!("_Something_ is missing")))
    Ok(entity)
  }
}
...
trait Repository {
  ...
  fn query_optional(&self, id: Self::IdType) -> Result<Option<Self::Entity>>;
  fn query(&self, id: Self::IdType) -> Result<Self::Entity> {
    let entity = self.query_optional(id).map_err(Err(anyhow!("Not found in {}", self.name())))
    Ok(entity)
  }
}
